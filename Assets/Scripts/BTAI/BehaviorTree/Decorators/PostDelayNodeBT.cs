...
{
    public class PostDelayNodeBT<T_ConfigurationBT> : DecoratorNodeBT<T_ConfigurationBT>
    where T_ConfigurationBT : IConfigurationBT
    {
        public int maxFrame { get; private set; }
        private int m_executedFrame;

        internal PostDelayNodeBT(T_ConfigurationBT config, NodeBT<T_ConfigurationBT> node, int frameCount)
        : base(config, node)
        {
            SetFrame(frameCount);
        }
