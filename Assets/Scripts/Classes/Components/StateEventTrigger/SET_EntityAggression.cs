...
namespace Unchord
{
    [AddComponentMenu("Unchord System/State Event Trigger/Entity Aggression Events (SET)")]
    public sealed class SET_EntityAggression : StateEventTrigger<IEntityAggressionEvents>
    {
        ...
        protected override void FixedUpdate()
        {
            ...
            {
                if(bAggroPrev)
                    iEventListener?.OnAggroEnd(this);
            }
            else
            {
                if(!bAggroPrev)
                    iEventListener?.OnAggroBegin(this);

                iEventListener?.OnAggressive(this);
            }
        }
    }
}