namespace Unchord
{
    ...
    {
        public static void SyncTransforms(Transform2 _transform)
        {
            Transform2 _root = _transform;

            while(_root.parent != null)
                _root = _root.parent;

            rec_SyncTransforms(_root, null);
        }

        public static void InheritTransform(Transform2 _child, Transform2 _parent)
        {
            if(_child == null)
                return;
            ...
            _child.parent?.children.Add(_child);
        }

        public static void UninheritTransform(Transform2 _transform)
        {
            if(_transform == null)
                return;

            _transform.parent?.children.Remove(_transform);
            _transform.parent = null;
        }

        public static void UnsafeSyncBasis(Transform2 _child, Transform2 _parent)
        {
        ...
        public static void UnsafeSyncRootBasis(Transform2 _root)
        {
            _root.gfx = _root.lfx;
            _root.gfy = _root.lfy;
        ...
        private static void rec_SyncTransforms(Transform2 _child, Transform2 _parent)
        {
            if(_child == null)
                return;
            else if(_parent == null)
                TransformManager2.UnsafeSyncRootBasis(_child);
            else
                TransformManager2.UnsafeSyncBasis(_child, _parent);

            int count = _child.children.Count;

            for(int i = 0; i < count; ++i)
                rec_SyncTransforms(_child.children[i], _child);
        }
    }
}